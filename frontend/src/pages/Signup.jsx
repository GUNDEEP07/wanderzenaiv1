...
export default function Signup() {
  ...
  const handleSignup = async (e) => {
    ...
    setError(''); setLoading(true);
    try { await signUpWithEmail(email, password); navigate('/onboarding'); }
    catch (err) {
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.box}>
        ...
        {isDemo && <div style={s.demo}>✦ Demo mode — create an account with any credentials</div>}
        ...
        <form onSubmit={handleSignup}>
          ...
          <button style={s.btn} type="submit" disabled={loading}>{loading ? '…' : 'Create account →'}</button>
        </form>
